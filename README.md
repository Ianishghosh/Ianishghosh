<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=0072ff,00c6ff&height=250&section=header&text=Anish%20Ghosh&fontSize=70&animation=fadeIn&fontAlignY=38&desc=Machine%20Learning%20Engineer%20%7C%20AI%20Researcher&descAlignY=55&descAlign=50&fontColor=ffffff" width="100%"/>
</div>

### 👨‍💻 Professional Summary

<p align="left">
 <img align="right" src="https://media.giphy.com/media/dummy/giphy.gif" width="0" /> I am a **Machine Learning Engineer & Researcher** specializing in Deep Learning, Audio Forensics, and Generative AI. My core expertise lies in developing highly optimized, production-ready AI pipelines and deploying machine learning models on resource-constrained edge devices. 
 
 🔭 **Current Focus:** Large Language Models (LLMs), Audio Transformers, and mastering Data Structures & Algorithms (DSA) for scalable system design.  
 🚀 **Goal:** Bridging the gap between complex AI research and high-efficiency, real-world software applications.  
 📫 **Contact:** ghoshanish.848@gmail.com | [LinkedIn](https://www.linkedin.com/in/anish-ghosh-284892282/)
</p>

---

### 🛠️ Technical Arsenal

| Domain | Technology Stack |
| :--- | :--- |
| **Languages** | ![Python](https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=white) ![C++](https://img.shields.io/badge/-C++-00599C?style=flat&logo=c%2B%2B&logoColor=white) ![SQL](https://img.shields.io/badge/-SQL-4479A1?style=flat&logo=mysql&logoColor=white) |
| **AI / Machine Learning** | ![PyTorch](https://img.shields.io/badge/-PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white) ![TensorFlow](https://img.shields.io/badge/-TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white) ![HuggingFace](https://img.shields.io/badge/-HuggingFace-FFD21E?style=flat&logo=huggingface&logoColor=black) ![Scikit-learn](https://img.shields.io/badge/-Scikit_Learn-F7931E?style=flat&logo=scikit-learn&logoColor=white) |
| **Frameworks & Edge** | ![FastAPI](https://img.shields.io/badge/-FastAPI-009688?style=flat&logo=fastapi&logoColor=white) ![Flask](https://img.shields.io/badge/-Flask-000000?style=flat&logo=flask&logoColor=white) ![React](https://img.shields.io/badge/-React-61DAFB?style=flat&logo=react&logoColor=black) ![Raspberry Pi](https://img.shields.io/badge/-Raspberry_Pi-C51A4A?style=flat&logo=raspberry-pi&logoColor=white) |
| **Tools & DSP** | ![Git](https://img.shields.io/badge/-Git-F05032?style=flat&logo=git&logoColor=white) ![Jupyter](https://img.shields.io/badge/-Jupyter-F37626?style=flat&logo=jupyter&logoColor=white) ![Librosa](https://img.shields.io/badge/-Librosa-4B8BBE?style=flat&logo=python&logoColor=white) ![OpenCV](https://img.shields.io/badge/-OpenCV-5C3EE8?style=flat&logo=opencv&logoColor=white) |

---

### 🧬 Featured AI Engineering Projects

| Project | Architecture & Impact | Tech Stack |
| :--- | :--- | :--- |
| **[City-Flow | Traffic Prediction System](https://github.com/Ianishghosh/City-Flow.git)** | **Production Forecasting.** Engineered an ST-GNN system across 207 road sensors with a 4.2 mph MAE and sub-second latency over 34,000+ timesteps. | `PyTorch` `FastAPI` `Streamlit` |
| **[Plant Care AI | Disease Diagnosis](https://github.com/Ianishghosh/Plant---Diesease-API.git)** | **Generative AI & Computer Vision.** Built a React app utilizing MobileNet V2 for inference, orchestrated with Groq LLMs to automatically generate structured treatment reports. | `MobileNet V2` `Groq LLM` `React` |
| **[Deepfake Audio Detection](https://github.com/Ianishghosh/Audio_Deepfake.git)** | **Edge AI Deployment.** Developed an offline detection system using PyWT/Librosa feature extraction, deployed securely on a Raspberry Pi 4. | `TF Lite` `Flask` `Librosa` |

---

### 💼 Professional Experience

**🚀 Tech Intern @ Emvo AI** *(Sep 2025 - Present)*
- Developed and deployed a language identification model for multilingual audio streams. 
- Implemented noise cancellation techniques in the audio processing pipeline, successfully reducing background interference by an estimated 30%.

**🔬 AI Research Intern @ IIEST Shibpur** *(May 2025 - July 2025)*
- Researched and developed AI models for audio forensic analysis.
- Built and optimized edge deployment models, achieving 95% accuracy on 500+ samples and improving overall system processing speed by 60%.

---

### 📊 Problem Solving & Engineering Metrics

<div align="center">
  <a href="https://leetcode.com/u/AnG_20/">
    <img src="https://leetcard.jacoblin.cool/AnG_20?theme=tokyonight&font=Inter&ext=activity" alt="LeetCode Stats" />
  </a>
</div>

<br/>

<div align="center">
  <a href="https://github.com/anuraghazra/github-readme-stats">
    <img height="160em" src="https://github-readme-stats.vercel.app/api?username=Ianishghosh&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide=stars"/>
  </a>
  <a href="https://github.com/anuraghazra/github-readme-stats">
    <img height="160em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Ianishghosh&layout=compact&theme=tokyonight"/>
  </a>
</div>
